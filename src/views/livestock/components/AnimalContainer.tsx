...
// import { useHealthyRecordsList } from '../../healthy&treatments/hooks/useHealthyRecordsList';

// const AnimalContainer : React.FC = () => {
//    const { id,history,selectedAnimal,loading}=useAnimalView()
//    const clinicalHookData = useHealthyRecordsList(selectedAnimal?.health_records || [], undefined, 'client');
...
import { useAnimalView } from '../hooks/useAnimalView';
import { useHealthyRecordsList } from '../../healthy&treatments/hooks/useHealthyRecordsList';
import { useBreedingList } from '../../reproduction/hooks/useBreedingList'; // Ensure this hook accepts initial array parameters

const AnimalContainer: React.FC = () => {
  const { id, history, selectedAnimal, loading } = useAnimalView();
  
  // Initialize hooks safely in client mode using the animal's local records
  const clinicalHookData = useHealthyRecordsList(selectedAnimal?.health_records || [], undefined, 'client');
  const breedingHookData = useBreedingList(selectedAnimal?.breeding_history || [], undefined, 'client');
  ...
  return (
    <Container maxWidth="lg">
      ...
      {/* Global Actions */}
      <Stack direction="row" spacing={2} sx={{ mt: 3, mb: 1 }} justifyContent="center">
        <Button variant="outlined" startIcon={<IonIcon icon={scaleOutline} />} onClick={() => history.push('/weights/add')}>Log Weight</Button>
        <Button variant="outlined" startIcon={<IonIcon icon={fitnessOutline} />} onClick={() => history.push('/health/add')}>Log Health</Button>
        <Button variant="outlined" startIcon={<IonIcon icon={heartOutline} />} onClick={() => history.push("/reproduction/add")}>Log Breeding</Button>
      </Stack>
