...
//                     {upcomingCalvings.count} Expected
//                   </Typography>
//                 </Stack>
//                 <Divider sx={{ mb: 1 }} />
                
//                 {upcomingCalvings.count > 0 ? (
//                   <Stack spacing={2}>
//                     {upcomingCalvings?.results?.slice(0, 3).map((event: any) => (
...
const BreedingBanner: React.FC = () => {
  // Pull states directly via our dual-mode custom hook context
  const { history, storedEventsRaw, upcomingCalvings } = useBreedingList([], undefined, 'server');

  // Safely grab the backend aggregation values, falling back to 0
  const confirmedPregnanciesCount = storedEventsRaw?.confirmed_pregnancies_count || 0;
  const expectedCalvingsCount = upcomingCalvings?.count || 0;
  ...
  return (
    <Box sx={{ p: 0, m: 0 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 1 }}>
        <Grid item xs={12} md={8}>
          <Paper elevation={0} sx={{ p: 3, borderRadius: '4px', border: '1px solid #ececec', bgcolor: '#ffffff' }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
              <Typography variant="body1" fontWeight="bold">Upcoming Calvings</Typography>
              <Typography variant="body2" fontWeight="bold" sx={{ color: '#18774c', bgcolor: '#e8f5e9', padding: '5px 10px', borderRadius: '4px' }}>
                {expectedCalvingsCount} Expected
              </Typography>
            </Stack>
            <Divider sx={{ mb: 2 }} />
            
            {expectedCalvingsCount > 0 ? (
              <Stack spacing={2}>
                {upcomingCalvings?.results?.slice(0, 3).map((event: any) => (
                  <Stack key={event.id} direction="row" justifyContent="space-between" alignItems="center">
                    <Box>
                      <Typography variant="subtitle2" fontWeight="bold">Tag: {event.dam_tag || event.animal_tag}</Typography>
                      <Typography variant="caption" color="text.secondary">
                        Exp. Date: {event.expected_calving_date}
                      </Typography>
                    </Box>
                    <Chip 
                      ...
                      size="small" 
                      clickable 
                      onClick={() => history.push(`/reproduction/view/${event.id}`)}
                      variant="outlined" 
                    />
                  </Stack>
                ))}
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={0} sx={{ p: 1, borderRadius: '4px', border: '1px solid #ececec', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', bgcolor: '#ffffff' }}>
            <IonIcon icon={heartOutline} style={{ fontSize: '2.5rem', color: '#eb445a', marginBottom: '10px' }} />
            <Typography variant="h5" fontWeight="bold" sx={{ color: '#18774c', mb: 0.5 }}>
              {confirmedPregnanciesCount}
            </Typography>
            <Typography variant="overline" color="text.secondary" fontWeight="bold">
              Confirmed Pregnancies
            </Typography>
