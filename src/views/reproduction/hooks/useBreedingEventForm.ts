...
export const useBreedingEventForm = (initialData?: any) => {
  ...
  const isFormValid = useMemo(() => {
    return formData.animal !== '' && formData.breeding_date !== '' && formData.method !== '';
  }, [formData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      if (id) {
        await dispatch(updateBreedingEvent({ id: Number(id), data: formData })).unwrap();
      } else {
        await dispatch(logBreedingEvent(formData)).unwrap();
      }
      history.replace("/reproduction");
  ...
  return { formData, setFormData, isSubmitting, handleSubmit, isFormValid };
};