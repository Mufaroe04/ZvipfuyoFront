...
const DairyDashboard: React.FC = () => {
  ...
  // --- Logic: Improved Aggregation with Strict Date Sorting ---
  const aggregateData = (data: any[], valueKeys: string | string[]) => {
    const stats = data.reduce((acc: any, curr) => {
      ...
      if (!acc[dateKey]) {
        acc[dateKey] = { 
          total: Array.isArray(valueKeys) ? valueKeys.map(() => 0) : 0, 
          count: 0 
        };
      }
      ...
      if (Array.isArray(valueKeys)) {
        valueKeys.forEach((key, idx) => {
          acc[dateKey].total[idx] += Number(curr[key]) || 0;
        });
      } else {
    ...
    // Crucial: Sort keys to ensure the chart line flows left-to-right
  ...
  const yieldSeries = useMemo(() => {
    ...
    return [{
      name: 'Avg Liters/Day',
      data: Object.keys(daily).map(date => ({
        x: new Date(date).getTime(),
        y: Number((daily[date].total / daily[date].count).toFixed(2))
      }))
    }];
  ...
  const qualitySeries = useMemo(() => {
    ...
    return [
      { 
        ...
        data: Object.keys(daily).map(date => ({
          x: new Date(date).getTime(), 
          y: Number((daily[date].total[0] / daily[date].count).toFixed(2))
        }))
      },
      { 
        name: 'Avg Protein %', 
        data: Object.keys(daily).map(date => ({
          x: new Date(date).getTime(), 
          y: Number((daily[date].total[1] / daily[date].count).toFixed(2))
        }))
      }
    ];
  }, [milkQuality, selectedBreed]);

  const getChartOptions = (yAxisLabel: string, colors: string[]): any => ({
    chart: { type: 'area', toolbar: { show: false }, zoom: { enabled: false } },
    colors: colors,
    fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.4, opacityTo: 0.1 } },
    stroke: { curve: 'smooth', width: 3 },
  ...
  const lactationColumns: GridColDef[] = [
    ...
    { 
      ...
      headerName: 'Status', 
      width: 150,
      renderCell: (params) => params.value ? "Dry" : "Milking"
    },
    {
      field: 'actions',
      headerName: 'Actions',
      ...
      renderCell: (params) => (
        !params.row.end_date && (
          <Button variant="outlined" size="small" onClick={() => handleDryOff(params.row.id)}>
            Mark Dry
          </Button>
        )
      )
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#f8f9fa', minHeight: '100vh' }}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems="center" spacing={2} sx={{ mb: 3 }}>
        <Box>
          <Typography variant="h5" fontWeight="bold">Dairy Intelligence</Typography>
          <Typography variant="body2" color="text.secondary">Herd productivity analysis</Typography>
        </Box>
      ...
      <CustomTabPanel value={tabValue} index={0}>
        <DairyTabContent 
          title="Herd Yield Trend" 
          series={yieldSeries} 
          options={getChartOptions('Liters', ['#3880ff'])} 
          rows={milkYields.filter(r => selectedBreed === 'All' || r.breed === selectedBreed)} 
          columns={yieldColumns}
          actionLabel="Add Yield"
          onAction={() => history.push("/dairy/milk-yield/add")}
        />
      </CustomTabPanel>
      ...
      <CustomTabPanel value={tabValue} index={1}>
        <DairyTabContent 
          title="Quality Trend" 
          series={qualitySeries} 
          options={getChartOptions('Percentage', ['#2dd36f', '#3880ff'])} 
          rows={milkQuality.filter(r => selectedBreed === 'All' || r.breed === selectedBreed)} 
          columns={qualityColumns}
          actionLabel="Add Quality"
          onAction={() => history.push("/dairy/milk-quality/add")}
        />
      </CustomTabPanel>

      <CustomTabPanel value={tabValue} index={2}>
        <DairyTabContent 
          title="Lactation Logs" 
          series={[]} // Empty for now or custom bar logic
          options={getChartOptions('Status', ['#77757d'])} 
          rows={lactations.filter(r => selectedBreed === 'All' || r.breed === selectedBreed)} 
          columns={lactationColumns}
          actionLabel="Add Lactation"
          onAction={() => history.push("/dairy/milk-lactation/add")}
        />
      </CustomTabPanel>
    </Box>
  );
...
const DairyTabContent = ({ title, series, options, rows, columns, actionLabel, onAction }: any) => (
  <Stack spacing={3}>
    ...
    <Box>
      <Stack direction="row" justifyContent="space-between" mb={2}>
        ...
        <Button variant="contained" size="small" onClick={onAction} sx={{ textTransform: 'none' }}>{actionLabel}</Button>
      </Stack>
      <Paper sx={{ height: 450, borderRadius: '16px', overflow: 'hidden' }}>
