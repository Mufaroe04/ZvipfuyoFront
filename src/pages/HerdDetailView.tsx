...
const HerdDetailView: React.FC = () => {
  ...
  const animalColumns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 90,
      renderCell: (p) => (
        <Button variant='outlined' size="small" onClick={() => history.push(`/animal/${p.id}`)}>Details</Button>
      )
    }
  ];
  ...
  return (
    <IonPage>
      ...
      <IonContent className="ion-padding">
        {/* Header Section */}
        <Box sx={{p:2, mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <Box>
            <Typography variant="h5" fontWeight="bold">{selectedHerd.name}</Typography>
